...
class Script(scripts.Script):
        ...
        with gr.Group():
            with gr.Accordion("Additional Networks", open=False):
                with gr.Row():
                    ...
                    separate_weights = gr.Checkbox(
                        label="Separate UNet/Text Encoder weights", value=False
                    )
                    ctrls.append(separate_weights)
                    self.infotext_fields.append(
                        (separate_weights, "AddNet Separate Weights")
                    )

                for i in range(MAX_MODEL_COUNT):
                    with gr.Row():
                        module = gr.Dropdown(
                            ["LoRA"], label=f"Network module {i+1}", value="LoRA"
                        )
                        model = gr.Dropdown(
                            list(lora_models.keys()), label=f"Model {i+1}", value="None"
                        )
                        with gr.Row(visible=False):
                            model_path = gr.Textbox(
                                value="None", interactive=False, visible=False
                            )
                        model.change(
                            lambda module, model, i=i: model_util.lora_models.get(
                                model, "None"
                            ),
                            inputs=[module, model],
                            outputs=[model_path],
                        ...
                        model_info = gr.Button(
                            value=memo_symbol,
                            elem_id=f"additional_networks_send_to_metadata_editor_{i}",
                        )
                        model_info.click(
                            fn=None,
                            _js="addnet_send_to_metadata_editor",
                            inputs=[module, model_path],
                            outputs=[],
                        )

                        module.change(
                            lambda module, model, i=i: xyz_grid_support.update_axis_params(
                                i, module, model
                            ),
                            inputs=[module, model],
                            outputs=[],
                        ...
                        weight.change(
                            lambda w: (w, w),
                            inputs=[weight],
                            outputs=[weight_unet, weight_tenc],
                        )
                ...
                def update_weight_sliders(separate, *sliders):
                    ...
                    for w, w_unet, w_tenc in zip(*(iter(sliders),) * 3):
                        if not separate:
                            ...
                            w_tenc = w
                        updates.append(
                            gr.Slider.update(visible=not separate)
                        )  # Combined
                        updates.append(
                            gr.Slider.update(visible=separate, value=w_unet)
                        )  # UNet
                        updates.append(
                            gr.Slider.update(visible=separate, value=w_tenc)
                        )  # TEnc
                    return updates

                separate_weights.change(
                    update_weight_sliders,
                    inputs=[separate_weights] + weight_sliders,
                ...
                def refresh_all_models(*dropdowns):
                    ...
                    for dd in dropdowns:
                        ...
                        update = gr.Dropdown.update(
                            value=selected, choices=list(lora_models.keys())
                        )
                ...
                refresh_models.click(
                    refresh_all_models, inputs=model_dropdowns, outputs=model_dropdowns
                )
    ...
    def set_infotext_fields(self, p, params):
        for i, t in enumerate(params):
            ...
            if (
                ...
                or model == "None"
                or len(model) == 0
                or (weight_unet == 0 and weight_tenc == 0)
            ):
                continue
    ...
    def process_batch(self, p, *args, **kwargs):
        ...
        models_changed = (
            len(self.latest_networks) == 0
        )  # no latest network (cleared by check-off)
        models_changed = (
            models_changed or self.latest_model_hash != p.sd_model.sd_model_hash
        )
        if not models_changed:
            for (l_module, l_model, l_weight_unet, l_weight_tenc), (
                module,
                model,
                weight_unet,
                weight_tenc,
            ) in zip(self.latest_params, params):
                if (
                    l_module != module
                    or l_model != model
                    or l_weight_unet != weight_unet
                    or l_weight_tenc != weight_tenc
                ):
                    models_changed = True
                    break
        ...
        if models_changed:
            ...
            for module, model, weight_unet, weight_tenc in self.latest_params:
                ...
                print(
                    f"{module} weight_unet: {weight_unet}, weight_tenc: {weight_tenc}, model: {model}"
                )
                if module == "LoRA":
...
def on_ui_tabs():
    ...
    return [
        (additional_networks_interface, "Additional Networks", "additional_networks")
    ]
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        "additional_networks_reverse_sort_order",
        shared.OptionInfo(False, "Reverse model sort order", section=section),
    )
    shared.opts.add_option(
        "additional_networks_model_name_filter",
        shared.OptionInfo("", "LoRA model name filter", section=section),
    )
    shared.opts.add_option(
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            1,
            "# of threads to use for hash calculation (increase if using an SSD)",
            section=section,
        ),
    )
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            True,
            "Make a backup copy of the model being edited when saving its metadata.",
            section=section,
        ),
    )
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            False, "Only show .safetensors format models", section=section
        ),
    )
    shared.opts.add_option(
    ...
    shared.opts.add_option(
        "additional_networks_max_top_tags",
        shared.OptionInfo(20, "Max number of top tags to show", section=section),
    )
    shared.opts.add_option(
...
def on_infotext_pasted(infotext, params):
    ...
    for i in range(MAX_MODEL_COUNT):
        ...
        params[f"AddNet Model {i+1}"] = str(
            model_util.find_closest_lora_model_name(params[f"AddNet Model {i+1}"])
        )

        xyz_grid_support.update_axis_params(
